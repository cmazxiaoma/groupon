var detailView = $.extend({}, $.fn.datagrid.defaults.view, {
    render: function (target, container, frozen) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            //add by road start 2014-01-19
            if (rows[i]['routerCall']) {
                if (!frozen) {
                    table.push('<tr id="' + rowId + '_btn">');
                    table.push('<td align="left" colspan="' + fields.length + '">');
                    table.push('<div class="datagrid-row">');
                    table.push(rows[i]['routerCall']);
                    table.push('</div>');
                    table.push('</td>');
                    table.push('</tr>');
                } else {
                    table.push('<tr id="' + rowId + '_btn">');
                    table.push('<td colspan="' + (fields.length + 2) + '"><div class="datagrid-row"></div></td>');
                    table.push('</tr>');
                }
            }
            //add by road end
